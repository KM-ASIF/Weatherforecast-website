const apiKey = "ba48ee2670ae4b9493894346250510"; // Your API key
...
function displayAQI(aqi) {
  if (!aqi) {
    ...
    elements.aqiInfo.innerHTML = `<strong>Air Quality (PM2.5):</strong> ${parseFloat(aqi.pm2_5).toFixed(2)}`;
  }
  elements.aqiInfo.classList.add("visible");
}

// Display UV index and humidity
...
function displayWind(degree, kph) {
  elements.windInfo.innerHTML = `
    ...
    <svg class="wind-arrow" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" aria-hidden="true">
      <path fill="white" d="M12 2L15 11H9L12 2Z"/>
      <rect x="11" y="11" width="2" height="11" fill="white"/>
    </svg>
  `;
...
// Display hourly forecast for current day
function displayHourlyForecast(hours) {
  ...
  const filteredHours = hours
    .filter(h => Number(h.time.split(' ')[1].split(':')[0]) >= nowHour)
    .slice(0, 12);
...
function renderTemperatureChart(forecastDays) {
  ...
  temperatureChart = new Chart(elements.temperatureChartCtx, {
    ...
    options: {
      ...
      scales: {
        y: { beginAtZero: false, ticks: { color: 'white', font: { size: 12 } } },
